...
usage() {
    cat <<EOF
Usage: ./remote_refresh.sh [options] <command> [args...]
...
Examples:
  ./remote_refresh.sh deploy
  ./remote_refresh.sh refresh
  ./remote_refresh.sh logs web
  ./remote_refresh.sh -H root@server -D /srv/kanban deploy
  IMAGE_TAG=v2025-01-01 ./remote_refresh.sh deploy
EOF
}
